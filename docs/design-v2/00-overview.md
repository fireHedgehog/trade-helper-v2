...
| **Timing** | `/timing/:symbol?` | The same rule drilled into one symbol: a broker-style multi-pane chart with entry/exit markers, a trade table, and standard performance metrics. |
...
All six pages are built and working end to end. Depth work remains (more
signal models, walk-forward tuning, an options-analysis page) — not new
surfaces. 55 backend pytest tests pass; the frontend type-checks and builds.
