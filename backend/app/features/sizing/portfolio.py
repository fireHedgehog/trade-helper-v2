...
from app.features.sizing.params import BOOKS, COSTS, GROUP_CAPS, SYMBOL_CAP, MAX_STALE_DAYS, WINDOWS
...
def metrics(curve, capital=100000.):
...
def simulate(data,scope,window,book,method,cost,capital=100000., progress=None):
    ...
    for t in range(start,len(dates)):
        if progress and (t-start) % 90 == 0: progress(t-start, len(dates)-start)
