"""Research fill generator derived from the current production engine.

Only the explicit initial/channel/trailing switches extend its trade rules.
Cash accounting lives in cash.py. Prepared indicators are reused read-only.
"""
...
def run(bars: list[dict], params: ResearchParams, prepared: dict | None = None, emit_daily: bool = False) -> EngineResult:
    n = len(bars)
    ...
    atr = prepared["atr"] if prepared else ind.wilder_atr(h, low, c, params.atr_len)
    dc_up_e, dc_dn_e = prepared["channels"][params.entry_len] if prepared else ind.donchian(h, low, params.entry_len)
    dc_up_x, dc_dn_x = prepared["channels"][params.exit_len] if prepared else ind.donchian(h, low, params.exit_len)
    ...
    if not emit_daily:
        return EngineResult(trades=trades, overlays=overlays)
